...
export const clienteCreateSchema = Joi.object({
    ...
    endereco: Joi.string().required(100),
    bairro: Joi.string().required(30),
    complemento: Joi.string().allow(''),
    cep: Joi.string().required(),
    Telefone: Joi.string().email().required(),
    email: Joi.string().email(30).required(),
    senha: Joi.string().min(6).required(30),
});
...
export const listarClienteCpf = async (req, res) => {
    try {
        ...
        const cliente = await clienteService.findCpf(cpf);
        if (!cliente) {
...
export const adicionarCliente = async (req, res) => {
    ...
    export const atualizarCliente = async (req, res) => {
        try {
            const { cpf } = req.params;
            const update = await cçlienteService.update(cpf, req.body);
            if (!updated) {
                return res.status(404).json({ error: 'Cliente não encontrado' });
            }
